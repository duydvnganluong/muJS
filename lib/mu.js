 ...
 *   mu-post               "true" to force POST method on a link.
...
var mu = new function() {
	...
	this._defaults = {
		...
		processForms: true,
		/** @type {boolean} Ghost mode (no history, no scroll). */
		ghost: false,
		/** @type {boolean} Ghost mode for HTTP redirections. */
		ghostRedirect: false,
		...
		title: "title",
		/** @type {boolean|null} Scroll to top. null = auto. */
		scrollToTop: null,
	...
	/** @type {Map} Prefetch cache (url => html string or null if in-flight). */
	 ...
	 * Checks for disabled state, target attribute, onclick/onsubmit handlers,
	 * and URL format (must start with "/" but not "//").
	...
	this._shouldProcess = function(el) {
		...
		if (el.getAttribute("mu") === "true" || el.getAttribute("data-mu") === "true")
			return (true);
		// Check URL: must start with / but not //
	...
	this._elemCfg = function(el) {
		...
		if ((v = mu._attr(el, "prefix")) !== null)
			cfg.urlPrefix = v;
		cfg.ghost = mu._attrBool(el, "ghost", cfg.ghost);
		cfg.ghostRedirect = mu._attrBool(el, "ghost-redirect", cfg.ghostRedirect);
		cfg.scrollToTop = mu._attrBool(el, "scroll-to-top", cfg.scrollToTop);
		cfg.morph = mu._attrBool(el, "morph", cfg.morph);
		cfg.transition = mu._attrBool(el, "transition", cfg.transition);
		cfg.post = mu._attrBool(el, "post", false);
		cfg.confirm = mu._attr(el, "confirm");
		cfg.patchGhost = mu._attrBool(el, "patch-ghost", true);
		return (cfg);
	};
	...
	this._onClick = function(e) {
		...
		if (!el || !mu._cfg.processLinks || !mu._shouldProcess(el))
			return;
		...
		var url = cfg._url || el.getAttribute("href");
	...
	this._onSubmit = function(e) {
		...
		var url = cfg._url || form.getAttribute("action");
		cfg._sourceElement = form;
		if (method === "post") {
			e.preventDefault();
			cfg.postData = new FormData(form);
			// POST forms: ghost by default, scroll to top in navigation mode
			if (!form.hasAttribute("mu-ghost") && !form.hasAttribute("data-mu-ghost"))
				cfg.ghost = true;
			if (cfg.scrollToTop === null && cfg.mode !== "patch")
				cfg.scrollToTop = true;
		} else {
			e.preventDefault();
	 ...
	 * Fetches the target URL in the background with low priority.
	...
	this._onMouseOver = function(e) {
		...
		if (mu._attrBool(el, "prefetch", true) === false)
			return;
		var url = mu._attr(el, "url") || el.getAttribute("href");
		if (!url || mu._prefetchCache.has(url))
			return;
		// Mark as in-flight to avoid duplicate fetches
		mu._prefetchCache.set(url, null);
		var fetchUrl = mu._cfg.urlPrefix ? mu._cfg.urlPrefix + url : url;
		fetch(fetchUrl, {
			headers: {"X-Requested-With": "mujs", "X-Mu-Prefetch": "1"},
			priority: "low"
		})
		.then(function(r) { return (r.ok ? r.text() : null); })
		.then(function(html) {
			if (html)
				mu._prefetchCache.set(url, html);
			else
				mu._prefetchCache.delete(url);
		})
		.catch(function() { mu._prefetchCache.delete(url); });
	};
	/**
	...
	this._onPopState = function(e) {
		...
		var cfg = Object.assign({}, mu._cfg);
		cfg.ghost = true;
		cfg.scrollToTop = false; // don't auto-scroll, we restore manually
	...
	this._loadExec = async function(url, cfg) {
		...
		if (mu._abortCtrl)
			mu._abortCtrl.abort();
		mu._abortCtrl = new AbortController();
		mu._showProgress();
		try {
			var html = null;
			var resp = null;
			...
			if (mu._prefetchCache.has(url) && mu._prefetchCache.get(url) !== null) {
				html = mu._prefetchCache.get(url);
				mu._prefetchCache.delete(url);
			} else {
				// Build fetch options
				var fetchOpts = {
					signal: mu._abortCtrl.signal,
					headers: {
						"X-Requested-With": "mujs",
						"X-Mu-Mode": cfg.mode
					}
				};
				if (cfg.postData) {
			...
			// History management
			if (cfg.mode === "patch") {
				// Patch mode: no history by default (patchGhost=true)
				if (!cfg.patchGhost) {
				...
				// Navigation mode
				var ghost = cfg.ghost || (resp && resp.redirected && cfg.ghostRedirect);
				if (!ghost && !cfg.postData) {
					mu._previousUrl = mu._lastUrl;
					mu._lastUrl = finalUrl;
					window.history.pushState({mu: true, url: finalUrl}, "", finalUrl);
				} else if (resp && resp.redirected && !cfg.ghostRedirect) {
					mu._previousUrl = mu._lastUrl;
					mu._lastUrl = finalUrl;
					window.history.pushState({mu: true, url: finalUrl}, "", finalUrl);
				} else {
			...
			if (cfg.mode !== "patch") {
				if (cfg._restoreScroll) {
					...
					var isGhost = cfg.ghost || (resp && resp.redirected && cfg.ghostRedirect);
					var shouldScroll = cfg.scrollToTop !== null ? cfg.scrollToTop : !isGhost;
					if (shouldScroll) {
	...
	this._renderPage = function(html, cfg) {
		...
		// Merge <head> elements (stylesheets, meta)
		mu._mergeHead(doc);
	...
	/**
	 * Merge <head> elements between the current page and the fetched document.
	 * Adds new stylesheets and meta tags, removes stale ones.
	 * Only processes: link[rel=stylesheet], style, meta[name], meta[property].
	 * @param	{Document}	doc	Parsed document.
	 */
	this._mergeHead = function(doc) {
		var selector = "link[rel='stylesheet'], style, meta[name], meta[property]";
		...
		for (var i = 0; i < oldEls.length; i++)
			oldMap.set(mu._elKey(oldEls[i]), oldEls[i]);
		// Add new elements, track new keys
		var newKeys = new Set();
		for (var i = 0; i < newEls.length; i++) {
			var key = mu._elKey(newEls[i]);
			newKeys.add(key);
			if (!oldMap.has(key))
				document.head.appendChild(newEls[i].cloneNode(true));
		}
		// Remove elements not present in the new page
	...
	/**
	 * Generate a unique key for a <head> element, used for diffing.
	 * @param	{Element}	el	DOM element (link, style, or meta).
