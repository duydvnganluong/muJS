...
var mu = mu || new function() {
	...
	this._shouldProcess = function(el) {
		...
		if (muUrl !== null) {
			if (!muUrl.startsWith("/") || muUrl.startsWith("//"))
				return (false);
			return (true);
		}
	...
	this._elemCfg = function(el) {
		...
		// Backward compatibility: mu-post="true" → method "post" (deprecated)
		if (!cfg.method && mu._attrBool(el, "post", false))
			cfg.method = "post";
	...
	this._onSubmit = function(e) {
		...
		if (method !== "get") {
			...
			// Non-GET forms: ghost by default, scroll to top in navigation mode
	...
	this._onMouseOver = function(e) {
		...
		.then(function(html) {
			if (html)
				mu._prefetchCache.set(url, {html: html, ts: Date.now()});
			else
	...
	/**
	 * Mouseout handler for link prefetch cancellation.
	 * Removes the prefetch cache entry when the cursor leaves a link,
	 * preventing stale prefetches from being consumed later.
	...
	this._triggerAction = function(el) {
		if (!mu._shouldProcess(el))
			return;
		var cfg = mu._elemCfg(el);
		var url = cfg._url || el.getAttribute("href") || el.getAttribute("action");
		if (!url)
			return;
		cfg._sourceElement = el;
		// Default to GET
		if (!cfg.method)
			cfg.method = "get";
		// Non-click/submit triggers default to ghost mode
		if (!el.hasAttribute("mu-ghost") && !el.hasAttribute("data-mu-ghost"))
			cfg.ghost = true;
	...
	this._loadExec = async function(url, cfg) {
		...
		try {
			...
			if (cached && cached.html && (Date.now() - cached.ts) < mu._cfg.prefetchTtl) {
				html = cached.html;
				mu._prefetchCache.delete(url);
			} else {
				// Build fetch options
	...
	this._renderPage = function(html, cfg) {
		...
		// Update page title (skip in ghost mode)
		if (!cfg.ghost)
