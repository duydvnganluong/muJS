 ...
 * POST forms: data is sent as FormData. History is disabled by default
...
var mu = mu || new function() {
	...
	this._elemCfg = function(el) {
		...
		if (cfg.mode !== "replace" && cfg.mode !== "update" && cfg.mode !== "patch") {
			if (!el.hasAttribute("mu-history") && !el.hasAttribute("data-mu-history"))
				cfg.history = false;
			if (!el.hasAttribute("mu-scroll") && !el.hasAttribute("data-mu-scroll") && cfg.scroll === null)
				cfg.scroll = false;
		}
		return (cfg);
	};
	 ...
	 * POST forms: data sent as FormData, history disabled by default.
	...
	this._onSubmit = function(e) {
		...
		if (method !== "get") {
			...
			if (!form.hasAttribute("mu-history") && !form.hasAttribute("data-mu-history"))
				cfg.history = false;
