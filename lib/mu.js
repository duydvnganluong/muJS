...
var mu = mu || new function() {
	 ...
	 * Deleting on mouseout caused double requests: mouseout fires on child
	 * element transitions (mouseout != mouseleave), clearing the cache before
	 * the click event could consume it.
	...
	this._loadExec = async function(url, cfg) {
		var fetchUrl = cfg.urlPrefix ? cfg.urlPrefix + url : url;
		// Save current scroll position (skip for trigger loads)
		if (!cfg._trigger)
			mu._saveScroll();
