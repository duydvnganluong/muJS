 ...
 *   csrf              (bool)    Auto-attach a CSRF header (read from a
 *                               cookie) on non-GET requests. Default: true.
 *   csrfCookie        (string)  Cookie name holding the CSRF token.
 *                               Default: "XSRF-TOKEN" (Laravel's default).
 *   csrfHeader        (string)  Header name to send the token in.
 *                               Default: "X-XSRF-TOKEN" (Laravel's default).
 *
 *
 * ============================================================================
 * 8. EVENTS
 ...
 *   </script>
 *
 * SECURITY NOTE: the "mu-validate" value is looked up as a global function
 * name (window[name]) and called directly. Only ever hardcode this
 * attribute in your own templates — never build its value from user input,
 * or a malicious value could be used to invoke an unintended global
 * function.
 *
 *
 * ============================================================================
...
var mu = mu || new function() {
	...
	this._defaults = {
		...
		/** @type {boolean} Auto-attach a CSRF header on non-GET requests, read from a cookie. */
		csrf: true,
		/** @type {string} Cookie name to read the CSRF token from (Laravel default). */
		csrfCookie: "XSRF-TOKEN",
		/** @type {string} Header name to send the CSRF token in (Laravel default). */
	...
	this.load = function(url, opts) {
		...
		// Security: never fall back to the raw URL when it isn't same-origin.
		// Falling back would let mu.load() fetch a cross-origin endpoint,
		// leaking the CSRF header (if the target allows CORS) and injecting/
		// executing whatever HTML it returns into the current page.
		var resolved = mu._resolveUrl(url);
		if (!resolved) {
			console.warn("[\u00b5JS] mu.load() blocked non-same-origin URL: " + url);
			return;
		}
		mu._loadExec(resolved, cfg);
	};
	/**
	...
	/**
	 * Check whether an absolute or relative URL is same-origin.
	 * Used to gate which external resources (script src, redirect targets)
	 * µJS is allowed to trust/execute automatically.
	 * @param	{string}	url	Raw URL (absolute or relative).
	 * @return	{boolean}	True if the URL resolves to the current origin.
	 */
	this._isSameOrigin = function(url) {
		if (!url)
			return (false);
		try {
			var parsed = new URL(url, document.baseURI);
	...
	/**
	 * Read a CSRF cookie (default name "XSRF-TOKEN", set by Laravel and
	 * similar frameworks) and return its decoded value.
	 * @param	{string}	[cookieName]	Cookie name to read. Default "XSRF-TOKEN".
	 * @return	{string|null}	Decoded token value, or null if absent.
	 */
	this._getCsrfToken = function(cookieName) {
		cookieName = cookieName || "XSRF-TOKEN";
		...
		try {
			...
			if (!html) {
				...
				if (method !== "get") {
					...
					// Auto-attach CSRF header from cookie (e.g. Laravel's XSRF-TOKEN),
					// same behavior as Axios/jQuery defaults. Only sent same-origin
					// since fetchUrl is always resolved via _resolveUrl beforehand.
					if (cfg.csrf) {
						var csrfToken = mu._getCsrfToken(cfg.csrfCookie);
						if (csrfToken)
							fetchOpts.headers[cfg.csrfHeader] = csrfToken;
					}
				}
				...
				if (resp.redirected && mu._isSameOrigin(resp.url))
	...
	this._mergeHead = function(doc) {
		...
		for (var i = 0; i < newEls.length; i++) {
			...
			if (newEls[i].tagName.toUpperCase() === "SCRIPT") {
				// Security: only auto-load/execute scripts that are inline or
				// point to a same-origin src. A same-origin fetch response should
				// never be able to pull in and run a script from a third-party
				// origin just by including it in a <head> fragment.
				var newSrc = newEls[i].getAttribute("src");
				if (newSrc && !mu._isSameOrigin(newSrc)) {
					console.warn("[µJS] Blocked cross-origin script in merged <head>: " + newSrc);
					continue;
				}
	...
	this._runScripts = function(container) {
		...
		for (var i = 0; i < scripts.length; i++) {
			...
			if (old.hasAttribute("src")) {
				...
				// Security: never auto-execute a script pulled from a
				// cross-origin src. A same-origin response should not be able
				// to force-load third-party JS just by including a <script src>.
				if (!mu._isSameOrigin(src)) {
					console.warn("[µJS] Blocked cross-origin script: " + src);
					continue;
				}
